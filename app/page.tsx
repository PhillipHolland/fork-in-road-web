...
import { useChat } from '@ai-sdk/react'; // Updated import to match package structure
...
export default function ChatPage() {
  ...
  // Handle keyboard height changes on mobile
  useEffect(() => {
    const handleResize = () => {
  ...
  // Adjust input section padding based on keyboard height
  useEffect(() => {
    if (inputSectionRef.current) {
      inputSectionRef.current.style.paddingBottom = `${keyboardHeight}px`;
    }
  }, [keyboardHeight]);

  const handleResetChat = () => {
    setMessages([]); // Clear all messages to reset the chat
  };
  ...
  return (
    <div className="chat-container">
      <style jsx>{`
        ...
        .send-button {
          position: absolute;
          right: 40px; /* Adjusted to make space for reset button */
          top: 50%;
        ...
        @media (max-width: 850px) {
          ...
          .send-button {
            right: 34px; /* Adjusted for mobile */
            width: 26px;
            height: 26px;
            font-size: 12px;
          }
